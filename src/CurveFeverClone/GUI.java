...
class GUI {
	...
	GUI(Stage _stage) {
        ...
        // not title text
        Text notTitle = new Text("TODO: Fix input stutter and other stuff ^^");
        notTitle.setFill(Paint.valueOf("WHITE"));
        notTitle.setTextOrigin(VPos.BOTTOM);
        notTitle.setTextAlignment(TextAlignment.CENTER);
        notTitle.setFont(Font.font("System", FontWeight.BOLD, 18));
        ...
        vbox.getChildren().addAll( title, notTitle );
        ...
        rootScene.setOnKeyPressed(event -> { if(!keyStack.contains(event.getCode())) keyStack.push(event.getCode()); });
